...
static uint8_t user_register_1 = 0x3A   ;       //0b00111010; 
static uint8_t heater_control_register = 0x00   ;       //0b00000000;
...
static int8_t r_reg(Si7021_registers_t reg)
{
  ...
  if(HAL_OK != HAL_I2C_Mem_Read(&Si7021_i2c, I2C_ADDR, cmd, 1, data, 1, SensorI2C_Timeout))
    return -1;
  else
    return 0;
}
...
static int8_t w_reg(uint8_t value, Si7021_registers_t reg)
{
  ...
  if(HAL_OK != HAL_I2C_Mem_Write(&Si7021_i2c, I2C_ADDR, cmd, 1, &value, 1, SensorI2C_Timeout))
    return -1;
  else
    return 0;
}
...
int8_t r_single_Si7021(float* data, Si7021_measurement_type_t type)
{
  ...
  if(HAL_OK != HAL_I2C_Master_Transmit(&Si7021_i2c, I2C_ADDR, &cmd, 1, SensorI2C_Timeout))
    return -1;

  if(HAL_OK != HAL_I2C_Master_Receive(&Si7021_i2c, I2C_ADDR, buffer, 2, SensorI2C_Timeout))
    return -1;
...
int8_t r_firmware_rev_Si7021()
{
  ...
  if(HAL_OK != HAL_I2C_Master_Transmit(&Si7021_i2c, I2C_ADDR, (uint8_t*)(&cmd), 2, SensorI2C_Timeout))
    return -1;

  if(HAL_OK != HAL_I2C_Master_Receive(&Si7021_i2c, I2C_ADDR, &data, 1, SensorI2C_Timeout))
    return -1;
