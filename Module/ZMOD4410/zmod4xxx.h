 ...
 *
 *  This function needs a pointer as a parameter and return tracking number.
 *  The tracking number is uint8_t type and 6 dimension array.
 *  Ex: uint8_t track_number[6];
 *      zmod_read_tracking_number(dev, track_number);
 *  If function return success, the variable is filled with tracking number of sensor
 *
 * @param [in] dev pointer to the device
...
/**
 * @brief Calculate Factor of sensor
 * @param [in] dev pointer to the device
 * @return error code
 * @retval 0 success
 */
 ...
 * @retval  0 success
 * @retval  "!= 0" error
 * @note    Before calling function, initialization data set has to be passed the dev->init_conf
 */
 ...
 * @retval  0 success
 * @retval  "!= 0" error
 * @note Before calling function, measurement data set has to be passed the dev->meas_conf
 */
